...
**Field stitching rule:** the fulfillment response only provides stock signal. `name`, `brand`, `currentPriceCents`, `regularPriceCents`, `canonicalUrl` in the intermediate `ProductResult` are taken from the item's existing DB row, not from the fulfillment response. Layer 1.6 (§6.9) may then replace those metadata fields with live GraphQL product data before `decide()` writes the item row.
...
### 6.9 J-code metadata fallback (Layer 1.6)

For SKUs that priceBlocks cannot resolve but fulfillment can stock-check, refresh product metadata through Best Buy GraphQL over GET:

`GET https://www.bestbuy.com/gateway/graphql?operationName=getProduct&variables={"skuId":"<SKU>"}&query=<url-encoded-query>`

**Transport:** identical to priceBlocks (§6.2): warmed `curl_chrome116` session jar. This avoids `/gateway/graphql` POST, which remains Akamai POST-tier blocked from the VPS.

**Response parsing:** `data.productBySkuId.name.short`, `brand`, `primaryImage.piscesHref`, `buyingOptions[].pdpUrl` / `url.skuSpecificUrl`, and `price.currentPrice` / `price.regularPrice` or displayable price aliases.

**Pipeline placement:** immediately after successful Layer 1.5 fulfillment. Merge GraphQL metadata into the fulfillment stock `ProductResult`, preserving the fulfillment `buttonState` and `purchasable` values.

**Failure handling:** metadata failure does not discard the stock result. The worker logs `graphql_metadata_failed=...` and applies the fulfillment-only result.
